...
export default function GuidedCapture({
  ...
  const liveFeedback = useCallback((): FeedbackMessage | null => {
    ...
    // model-loading: do not block UI — capture is allowed; show subtle indicator below
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      ...
      {state.isStreaming && cameras.length > 1 && (
        <div className="absolute top-[88px] left-4 z-30">
      ...
      {/* Small model-loading indicator — non-blocking */}
      {state.isStreaming && detection.modelLoading && (
        <div className="absolute top-[88px] right-4 z-30 flex items-center gap-1.5 bg-black/50 backdrop-blur text-white/70 text-[11px] px-2.5 py-1.5 rounded-lg">
          <svg className="w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
          </svg>
          <span>AI ready soon…</span>
        </div>
      )}
